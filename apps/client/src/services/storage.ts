...
export const storage = {
  ...
  validateImportData: (raw: any): { valid: boolean; projects?: Project[]; error?: string } => {
    ...
    if (!Array.isArray(list)) {
      return { valid: false, error: 'El archivo JSON debe contener un arreglo de proyectos o un objeto exportado con la propiedad "projects".' };
    }
    if (list.length === 0) {
