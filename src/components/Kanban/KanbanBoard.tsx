...
export const KanbanBoard: React.FC<KanbanBoardProps> = ({
  ...
  const renderColumns = (statuses: typeof normalStatuses) => {
    return statuses.map((statusConfig) => {
      const colProjects = filteredProjects.filter(p => p.status === statusConfig.id);
      return (
        <KanbanColumn
          key={statusConfig.id}
  ...
  const alertProjectsCount = filteredProjects.filter(p => ['pendiente_revision', 'retrasado', 'pausado'].includes(p.status)).length;
